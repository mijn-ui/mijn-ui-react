...
const Navbar = () => {
  ...
  const renderPages = PAGES.map((page) => (
    <Link
      key={page.title}
      className="text-small text-muted-text hover:text-secondary"
      href={page.href}
    >
      {page.title}
    </Link>
    ...
    <header className="sticky inset-x-0 top-0 z-50 h-[var(--navbar-height)] w-full flex-col items-center justify-center border-b bg-transparent backdrop-blur-md md:flex">
      <nav className="flex w-full items-center justify-between px-5 py-2">
        <div className="flex items-center gap-4">
          ...
          <Badge className="bg-primary/20 text-tiny text-primary hover:bg-primary/20">
            v0.0.1
          </Badge>
        ...
        <div className="flex items-center md:hidden">
          ...
          <ClickAwayListener onClickAway={() => setDropdownOpen(false)}>
            <Collapsible open={dropdownOpen} onOpenChange={setDropdownOpen}>
              ...
              <CollapsibleContent className="top-[calc(var(--navbar-height)] absolute inset-x-0 mt-2 overflow-hidden bg-surface text-small transition-[height] data-[state=closed]:animate-collapsible-close data-[state=open]:animate-collapsible-open">
