...
<Alert 
  color="info"
  title="Note:"
  description="For components like `label` and `badge` that only have one slot, you cannot use the `classNames` prop to style the slots. Instead, you can pass the `className` prop to customize the style of the component. An example of this has already been shown above."
/>
