...
<Alert
  color="info"
  title="Note:"
  description="If you're using pnpm with a monorepo architecture, ensure your configuration points to the ROOT `node_modules` directory."
/>
...
export default {
  ...
  plugins: [animationPlugin, mijnUiPreset()], // [!code highlight]
}
```
  ...
  <Alert
    ...
    description={
    <span>
      For more details on customizing colors, check out the{" "}
      <Link className="underline decoration-primary text-foreground" href="/docs/customization/override-styles">
        Colors
      </Link>{" "}
      section.
    </span>
