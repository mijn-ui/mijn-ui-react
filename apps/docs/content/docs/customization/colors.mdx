...
import DefaultColorSwatches from "@/app/components/default-color-swatches";
...
<Alert
  title="Important"
  description="When customizing colors, make sure to use only HSL values without color functions (e.g: `hsl()`). Using functions will cause the theme system to fail."
  color="danger"
/>
